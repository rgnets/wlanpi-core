...
class NetworkNamespace():
    ...
    def destroy_namespace(namespace_name: str):
        ...
        NetworkNamespace._static_logger.info(f"Asked to destroy namespace {namespace_name}")
        NetworkNamespace._static_logger.info(f"Killing old processes in {namespace_name}")
        ...
        NetworkNamespace._static_logger.info(f"Moving interfaces out of {namespace_name}")

        for interface in NetworkNamespace.get_interfaces_in_namespace(namespace_name):
            print(interface)
            NetworkNamespace._static_logger.info(f"Moving interface {interface} out of {namespace_name}")
            ...
            if interface['name'].startswith('wlan'):
                ...
                NetworkNamespace._static_logger.info(f"Ignoring loopback interface {interface['name']}")
            else:
